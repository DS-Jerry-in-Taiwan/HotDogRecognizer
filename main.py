...
def __name__ == "__main__":
    ...
    parser_train.add_argument("--config", type=str, required=True, defualt= 'config.yaml',help="trainning configuration file path")
    ...
    parser_infer.add_argument("--device", type=str, defualt='cpu', help="Device to run inference on (cpu or cuda)")
    ...
    if hasattr(args, "train"):
        train(args)
    elif hasattr(args, "infer"):
        infer(args)
    elif hasattr(args, "export"):
        export(args)
    elif hasattr(args, "deploy"):
        deploy(args)
    else:
        parser.print_help()
